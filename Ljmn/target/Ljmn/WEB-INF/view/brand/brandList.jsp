...
function getData(){
	...
	var pam = {"cpage":cpage,"pageSize":pageSize,"brandName":brandName};
	$.ajax({
		url:"<%=request.getContextPath()%>/brand/findBrandByPage",
		dataType:"json",
		type:"post",
		data:pam,
		success:function(res){
			...
			var tabHead  = [{"data":"选择"},{"data":"品牌名称"},{"data":"添加时间"},{"data":"操作"}];
			...
			$("button[name='shanchu']").click(function(){
				...
				$.ajax({
					url:"<%=request.getContextPath()%>/brand/deleteBrand",
					data:{id:id},
					dataType:"json",
					type:"post",
					success:function(res){
			...
			$("button[name='xiugai']").click(function(){
				var aaa=$(this).val();
				location.href="<%=request.getContextPath()%>/brand/updateJsp?aaa="+aaa;
			})
...
<nav class="navbar navbar-default" role="navigation">
	...
	<div style="float: left;"><!-- 上 左 右 -->
		<div class="input-group">
			...
			<input type="text" id="brandName" class="form-control" placeholder="请输入品牌名" style="width: 130px"/>
		</div>
	</div>
